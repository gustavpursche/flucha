...
export default (el, result) => {
  ...
  const graphMaxY = Math.max(indexedData[medianYear] * 2, maxY + (maxY - graphMinY));
  ...
  if(!state[key].yourData) {
      state[key].yourData = data.map(d => ({year: d.year, value: indexedData[medianYear], defined: false}))
          .filter(d => {
